...
export default function NewCertificatePage({ user }) {
  ...
  const handleFileChange = (e, pageIndex) => {
    ...
    const compressImage = (imageFile, maxWidth = 1200, quality = 0.75) => {
    ...
    compressImage(file).then((compressedResult) => {
      if (pageIndex === 1) {
        setPage1File(file);
        setPage1Preview(compressedResult);
        setPage1FileName(file.name);
      } else {
        setPage2File(file);
        setPage2Preview(compressedResult);
